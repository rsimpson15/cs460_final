...
namespace final.Views
{
    public class ItemsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "ItemID,SellerID,Name,Description,Done")] Item item)
        {
            var lastID = db.Items.Take(10).Select(m => m.ItemID).ToList().LastOrDefault();
            if (ModelState.IsValid)
            {
                item.ItemID = lastID + 1;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ItemID,SellerID,Name,Description")] Item item)
        {
            if (ModelState.IsValid)
            {
